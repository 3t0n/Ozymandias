...
static int trader_get_sell_resource(int warehouse_id, int city_id) {
    ...
    while (imp < RESOURCE_MAX[get_game_engine()] && !empire_can_import_resource_from_city(city_id, resource_to_import)) {
        imp++;
        resource_to_import = city_trade_next_caravan_import_resource();
    }
    if (imp >= RESOURCE_MAX[get_game_engine()])
        return 0;
...
int figure::get_closest_warehouse(int x, int y, int city_id, int distance_from_entry, map_point *warehouse) {
    int exportable[RESOURCE_MAX[get_game_engine()]];
    int importable[RESOURCE_MAX[get_game_engine()]];
